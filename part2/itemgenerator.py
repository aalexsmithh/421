...
managers = [1247,1296,1287,1265]
cashiers = [1249,1250,1251,1252,1253,1267,1268,1268,1270]
deliverydrivers = [1271,1278,1255,1259,1260,1274]
employees = [1247,1296,1287,1265,1249,1250,1251,1252,1253,1267,1268,1268,1270,1271,1278,1255,1259,1260,1274]
members = range(1001,1101)
...
def generateshifts():
	# shifts(shiftDate date, employeeID int, wage int, managerID int)

	week_start = np.datetime64("2018-02-01")
	for i in range(3):
		for employeeID in employees:
			date = week_start + np.random.choice(np.arange(0, 7))

			if employeeID in managers:
				managerID = employeeID
			else:
				managerID = np.random.choice(managers)

			wage = np.around(np.random.normal(25, 5, 1)[0],2)

			print '%s,%s,%s,%s' % (str(date), str(employeeID), str(wage), str(managerID))

		week_start = week_start + 12#np.timedelta64(8, 'D')
...
def generatedeliveryvehicle():
	...
	for i in range(len(deliverydrivers)):
		l = np.random.choice(license, size=(7), replace=False)
		out = ""
		for letter in l:
			out += letter
		
		d_id = deliverydrivers[i]

		print 'insert into deliveryvehicles values ("%s", %s)' % (str(out), str(d_id))
...
def generatecoupons():
	# coupons(couponCode int, pName text, discountPct int, startDate date, endDate date)
	with open('csv/items.csv', 'rb') as f:
		reader = csv.reader(f)
		items = np.asarray(list(reader))

	dsct_items = items[np.random.randint(items.shape[0], size=60), :]

	for i, d_i in enumerate(dsct_items):

		code = i + 1000
		pName = d_i[0]
		dsct_p = np.random.randint(10,75)
		start_d = np.datetime64("2018-01-01") + np.random.choice(np.arange(0, 90))
		end_d = start_d + 12
...
def generateordercontents(ordernum, items):
	# ordercontents(orderNo int, pName text, quantity int);

	ordercontents = []
	number = []
	for o_i in items:
		if o_i not in ordercontents:
			ordercontents.append(o_i)
			number.append(1)
		else:
			number[ordercontents.index(o_i)] += 1

	for i, oc in enumerate(ordercontents):
		print '%s,"%s",%s' % (str(ordernum), str(oc[0]), str(number[i]))

def generatetransaction(ordernum, total):
	# transactions(transactionID int, amount real, transactionDate date, cashierID int);

	cash_id = np.random.choice(cashiers, 1)

	date = np.datetime64("2018-03-01") - np.random.choice(np.arange(0, 30))
	date = str(date)[:10]

	print '%s,%s,%s,%s' % (str(ordernum), str(total), str(date), str(cash_id))

def generateorders():
	# orders(orderNo int, memberNo int, total real, toBeDelivered boolean, driverID int);
	...
	for i in range(25):
		num_items = np.round(np.random.normal(10, 8, 1))
		if num_items < 0:
			num_items = num_items - 2 * num_items

		order_items = items[np.random.randint(items.shape[0], size=int(num_items[0])), :]

		total = 0.0
		for o_i in order_items:
			total += float(o_i[1])

		tbd = np.random.choice([True,False])

		mem_no = np.random.choice(members)

		d_id = np.random.choice(deliverydrivers)

		print 'order %i' % (i)
		print '%s,%s,%s,%s,%s' % (str(i), str(mem_no), str(total), str(tbd), str(d_id))
		print
		generateordercontents(i, order_items.tolist())
		print
